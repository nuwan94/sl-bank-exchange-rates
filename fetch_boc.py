...
# NOTE: boc.lk's WAF returns 403 to requests from GitHub Actions' IP ranges
# specifically (confirmed via a headless-browser fetch getting the same 403
# from that same environment — this isn't a client-fingerprint thing, it's
# an IP-level block). Fails cleanly and shows up in debug_errors.json; the
# rest of the pipeline already handles a missing bank gracefully. Would need
# a self-hosted runner or a residential proxy to actually fetch this bank
# from CI.
